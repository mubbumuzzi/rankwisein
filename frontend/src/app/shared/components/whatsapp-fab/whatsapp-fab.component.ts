  ...
  template: `
    @if (visible()) {
      <a
        ...
        [class]="positionClass()"
        class="fixed z-40 inline-flex max-w-[calc(100vw-2rem)] items-center gap-2 rounded-full bg-[#25D366] px-4 py-3 text-sm font-bold text-white shadow-lg shadow-emerald-600/30 transition hover:scale-[1.02] hover:bg-[#20BD5A] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2"
        [attr.aria-label]="whatsappCtaLabel"
      >
        ...
        <span class="truncate">{{ whatsappCtaLabel }}</span>
      </a>
    }
  `,
})
